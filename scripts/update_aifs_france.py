#!/usr/bin/env python3
"""Construit les cartes et prévisions AIFS/ECMWF 0,25° pour la France.

La chaîne lit les données ouvertes GRIB2 officielles de l'ECMWF (modèle AIFS
Single, IA déterministe). Les fichiers nationaux sont découpés par département
pour WordPress et les cartes restent calculées depuis la grille AIFS, jamais
depuis les seules communes.

AIFS ne publie ni rafales (10fg), ni CAPE (mucape), ni réflectivité radar
dérivée, ni taux de précipitation instantané (tprate) : à la différence du
module CEP/IFS, ce script ne calcule donc ni carte de rafales, ni CAPE, ni
diagnostic orage/grêle/foudre. Les colonnes et couches correspondantes sont
absentes du produit plutôt que de publier une valeur toujours nulle qui
laisserait croire à une absence de risque.
"""
...
from aifs_maps import DEFAULT_BOUNDS, AIFSMapRenderer
...
DEFAULT_CURRENT_METADATA_URL = (
    ...
    "aifs/data/index.json"
)
USER_AGENT = "alertes-meteo.com/aifs-ecmwf-france/1.0.0"

# Grille mondiale régulière AIFS Open Data 0,25° (identique à IFS Open Data).
AIFS_NI = 1440
AIFS_NJ = 721
AIFS_LAT_FIRST = 90.0
AIFS_LON_FIRST = -180.0
AIFS_STEP = 0.25

MAP_WIDTH = 2200
MAP_HEIGHT = 1640

# Format compact partagé avec le JavaScript. Contrairement au module CEP/IFS,
# aucune colonne orage/rafales/CAPE n'existe ici : ces champs ne sont jamais
# publiés par AIFS Single.
VALUE_COLUMNS = (
    "temperature_c",
    "humidity_pct",
    "precipitation_mm",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=360,
        help="Dernière échéance AIFS, multiple de 6 entre 6 et 360 heures",
    )
    parser.add_argument(
...
def grid_index(latitude: float, longitude: float) -> tuple[int, float, float]:
    row = int(round((AIFS_LAT_FIRST - latitude) / AIFS_STEP))
    column = int(round(((longitude - AIFS_LON_FIRST) % 360.0) / AIFS_STEP)) % AIFS_NI
    row = max(0, min(AIFS_NJ - 1, row))
    column = max(0, min(AIFS_NI - 1, column))
    index = row * AIFS_NI + column
    model_latitude = AIFS_LAT_FIRST - row * AIFS_STEP
    model_longitude = AIFS_LON_FIRST + column * AIFS_STEP
...
def load_catalog(path: Path) -> NationalCatalog:
    ...
    LOGGER.info(
        "Catalogue AIFS : %s communes, %s points AIFS 0,25°, %s départements",
        len(raw_communes),
        len(model_indexes),
        len(departments),
    )
    return NationalCatalog(
        version=f"{payload.get('catalog_version', '1')}-aifs001",
...
class NationalGrid:
    ...
    def validate(self, gid: int) -> None:
        ...
        if (
            ni != AIFS_NI
            or nj != AIFS_NJ
            or not math.isclose(lat_first, AIFS_LAT_FIRST, abs_tol=1.0e-6)
            or not math.isclose(lon_first, AIFS_LON_FIRST, abs_tol=1.0e-6)
        ):
            raise RuntimeError(
                "La grille reçue n'est pas AIFS Open Data 0,25° "
                f"({ni} × {nj}, premier point {lat_first}/{lon_first})"
            )
        if max(self.catalog.model_indexes) >= ni * nj:
            raise RuntimeError("Un indice communal dépasse la grille AIFS")
        self.validated = True
...
class MapSampler:
    """Rééchantillonne la grille AIFS par spline bicubique sur Web Mercator."""
    ...
    def __init__(self, width: int, height: int) -> None:
        ...
        rows = (AIFS_LAT_FIRST - target_latitudes) / AIFS_STEP
        columns = (
            (target_longitudes - AIFS_LON_FIRST) % 360.0
        ) / AIFS_STEP
...
AIFS_PARAMETERS = ["2t", "2d", "10u", "10v", "msl", "sp", "tcc", "tp", "sf", "z"]


def forecast_steps(forecast_hours: int) -> list[int]:
    """Échéances déterministes AIFS : 6 h jusqu'à 360 h."""
    return list(range(0, forecast_hours + 1, 6))


def retrieve_aifs_step(
    client: Client,
    run_time: datetime,
    lead: int,
    destination: Path,
...
def build_product(
    ...
    try:
        ...
        for step_number, lead in enumerate(steps):
            ...
            try:
                destination = downloads / f"aifs-{lead:03d}h.grib2"
                LOGGER.info("Téléchargement AIFS +%03d h", lead)
                retrieve_aifs_step(client, run_hint, lead, destination)
                source_bytes += destination.stat().st_size
                current_paths.append(destination)
                LOGGER.info(
                    "Décodage et cartes AIFS %s/%s : +%03d h",
                    step_number + 1,
                    len(steps),
                    lead,
                )
    ...
    model = {
        "name": "AIFS / ECMWF déterministe (IA)",
        "provider": "ECMWF",
        "dataset": "ECMWF Open Data — AIFS Single",
    ...
    with (result_directory / "index.json").open("w", encoding="utf-8") as handle:
        ...
        handle.write("\n")
    LOGGER.info(
        "Produit AIFS prêt : %.1f Mo de tableaux, %s couches, %s échéances",
        total_size / 1e6,
...
def main() -> int:
    ...
    if not 6 <= args.forecast_hours <= 360 or args.forecast_hours % 6 != 0:
        raise ValueError(
            "forecast-hours doit être un multiple de 6 compris entre 6 et 360"
        )
    ...
    LOGGER.info("Run AIFS sélectionné : %s", iso_utc(run_hint))
    if not args.force and already_published(
        args.current_metadata_url, run_hint
    ):
        LOGGER.info("Ce run AIFS est déjà publié ; aucune reconstruction nécessaire")
        return 0

    with tempfile.TemporaryDirectory(
        prefix="aifs-france-build-", ignore_cleanup_errors=True
    ) as temporary:
...
if __name__ == "__main__":
    try:
        ...
        LOGGER.exception("Échec de la mise à jour AIFS France")
        raise SystemExit(1)
