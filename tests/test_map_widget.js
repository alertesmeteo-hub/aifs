...
class Element {
    ...
    querySelectorAll(selector) {
        const found = [];
        const visit = element => {
            if (selector === '[data-aifsm-layer-key]' && element.dataset.aifsmLayerKey) {
                found.push(element);
            }
...
app.querySelector = selector => {
    const match = selector.match(/^\[data-aifsm-([^\]]+)\]$/);
...
const documentMock = {
    ...
    querySelectorAll(selector) { return selector === '[data-aifsm-app]' ? [app] : []; },
...
const scriptPath = path.resolve(__dirname, '../wordpress/aifs-ecmwf-france/assets/aifs-map.js');
...
(async () => {
    ...
    app.dispatch('aifsm:focus-location', {
    ...
    const layerButtons = elements['layer-grid'].querySelectorAll('[data-aifsm-layer-key]');
    const rainPeriod = layerButtons.find(button =>
        button.dataset.aifsmLayerKey === 'pluie_cumul');
    ...
    const gustPeriod = layerButtons.find(button =>
        button.dataset.aifsmLayerKey === 'rafales_max');
