(function () {
    ...
    function initMap(app) {
        ...
        var menuToggle = app.querySelector('[data-aifsm-menu-toggle]');
        var menuClose = app.querySelector('[data-aifsm-menu-close]');
        var menuCloseLabel = app.querySelector('[data-aifsm-menu-label]');
        var menuCloseIcon = app.querySelector('[data-aifsm-menu-icon]');
        var layerMenu = app.querySelector('[data-aifsm-layer-menu]');
        var layerGrid = app.querySelector('[data-aifsm-layer-grid]');
        var currentLayerText = app.querySelector('[data-aifsm-current-layer]');
        var previousButton = app.querySelector('[data-aifsm-previous]');
        var playButton = app.querySelector('[data-aifsm-play]');
        var nextButton = app.querySelector('[data-aifsm-next]');
        var validity = app.querySelector('[data-aifsm-validity]');
        var lead = app.querySelector('[data-aifsm-lead]');
        var run = app.querySelector('[data-aifsm-run]');
        var generated = app.querySelector('[data-aifsm-generated]');
        var stale = app.querySelector('[data-aifsm-stale]');
        var viewport = app.querySelector('[data-aifsm-viewport]');
        var weatherCanvas = app.querySelector('[data-aifsm-weather]');
        var vectorCanvas = app.querySelector('[data-aifsm-vectors]');
        var labelsCanvas = app.querySelector('[data-aifsm-labels]');
        ...
        var mapTitle = app.querySelector('[data-aifsm-map-title]');
        var mapRun = app.querySelector('[data-aifsm-map-run]');
        var mapDate = app.querySelector('[data-aifsm-map-date]');
        var loading = app.querySelector('[data-aifsm-loading]');
        var errorBox = app.querySelector('[data-aifsm-error]');
        var slider = app.querySelector('[data-aifsm-slider]');
        var timeline = app.querySelector('[data-aifsm-timeline]');
        var singleTimeline = app.querySelector('[data-aifsm-single-timeline]');
        var periodSelector = app.querySelector('[data-aifsm-period]');
        var dualRange = app.querySelector('[data-aifsm-dual-range]');
        var periodStartSlider = app.querySelector('[data-aifsm-period-start]');
        var periodEndSlider = app.querySelector('[data-aifsm-period-end]');
        var periodTitle = app.querySelector('[data-aifsm-period-title]');
        var periodSummary = app.querySelector('[data-aifsm-period-summary]');
        var periodStartLabel = app.querySelector('[data-aifsm-period-start-label]');
        var periodEndLabel = app.querySelector('[data-aifsm-period-end-label]');
        var legend = app.querySelector('[data-aifsm-legend]');
        var zoomIn = app.querySelector('[data-aifsm-zoom-in]');
        var zoomOut = app.querySelector('[data-aifsm-zoom-out]');
        var reset = app.querySelector('[data-aifsm-reset]');
        var fullscreen = app.querySelector('[data-aifsm-fullscreen]');
        var zoomLevel = app.querySelector('[data-aifsm-zoom-level]');
        var probe = app.querySelector('[data-aifsm-probe]');
        var probeValue = app.querySelector('[data-aifsm-probe-value]');
        var probeLabel = app.querySelector('[data-aifsm-probe-label]');
        var toolButtons = app.querySelectorAll('[data-aifsm-tool]');
        var toolHint = app.querySelector('[data-aifsm-tool-hint]');
        var advancedTools = app.querySelector('[data-aifsm-advanced-tools]');
        var captureButton = app.querySelector('[data-aifsm-capture]');
        var copyButton = app.querySelector('[data-aifsm-copy]');
        var diagramPopup = app.querySelector('[data-aifsm-diagram-popup]');
        var diagramTitle = app.querySelector('[data-aifsm-diagram-title]');
        var diagramBody = app.querySelector('[data-aifsm-diagram-body]');
        var diagramStatus = app.querySelector('[data-aifsm-diagram-status]');
        var diagramClose = app.querySelector('[data-aifsm-diagram-close]');
        ...
        function pinProbeAt(clientX, clientY) {
            ...
            pinnedElement.className = 'aifsm-probe aifsm-probe-pinned';
            ...
            close.type = 'button';
            close.className = 'aifsm-probe-pin-close';
        ...
        function setToolMode(mode) {
            ...
            toolButtons.forEach(function (button) {
                var active = button.dataset.aifsmTool === toolMode;
            ...
            if (toolMode === 'diagram') {
                setToolHint('Cliquez sur la carte pour afficher le diagramme AIFS du point choisi.');
            } else {
                setToolHint('');
                closeDiagram();
            }
        ...
        function composeCaptureCanvas() {
            ...
            context.textAlign = 'left';
            context.fillText('ECMWF AIFS (IA) • FRANCE MÉTROPOLITAINE • 0,25°', 16, 22);
            context.fillStyle = '#ffffff';
            context.font = '900 24px Inter, Segoe UI, Arial, sans-serif';
            context.fillText(
                fitCaptureText(context, mapTitle ? mapTitle.textContent : 'Carte AIFS', width * 0.64),
                16,
                52
            );
            context.textAlign = 'right';
            ...
            context.textAlign = 'center';
            context.fillText(
                'www.alertes-meteo.com • Carte AIFS • Module v' + moduleVersion,
                width / 2,
        ...
        function captureImage() {
            ...
            canvas.toBlob(function (blob) {
                ...
                link.href = url;
                link.download = 'aifs-' + (slug || 'carte') + '-' + Date.now() + '.png';
        ...
        function renderDiagramChart(name, forecastRows, columnIndex, pointIndex) {
            ...
            svg.setAttribute('class', 'aifsm-diagram-svg');
            svg.setAttribute('role', 'img');
            svg.setAttribute('aria-label', 'Diagramme AIFS pour ' + name);
            ...
            rains.forEach(function (value, index) {
                ...
                rect.setAttribute('class', 'aifsm-diagram-rain');
            ...
            if (points.length > 1) {
                ...
                polyline.setAttribute('class', 'aifsm-diagram-temp');
            ...
            [0, count - 1].forEach(function (index) {
                ...
                text.setAttribute('class', 'aifsm-diagram-axis');
            ...
            caption.className = 'aifsm-diagram-caption';
            caption.textContent = 'Température (ligne) et précipitations par pas (barres) — prochaines échéances AIFS.';
        ...
        function refreshLayerMenu() {
            ...
            layerGrid.querySelectorAll('[data-aifsm-layer-key]').forEach(function (button) {
                var active = button.dataset.aifsmLayerKey === currentLayer;
                button.classList.toggle('is-active', active);
                button.setAttribute('aria-pressed', active ? 'true' : 'false');
            });
        }
        ...
        function buildLayerMenu() {
            ...
            groupOrder.forEach(function (group) {
                ...
                section.className = 'aifsm-layer-group';
                ...
                grouped[group].forEach(function (entry) {
                    ...
                    button.type = 'button';
                    button.className = 'aifsm-layer-option';
                    button.dataset.aifsmLayerKey = entry.key;
        ...
        function buildLegend() {
            ...
            strip.className = 'aifsm-legend-strip';
            layer.stops.forEach(function (stop) {
                var item = document.createElement('div');
                item.className = 'aifsm-legend-stop';
        ...
        function updatePeriodControls() {
            ...
            if (dualRange && dualRange.style && dualRange.style.setProperty) {
                ...
                dualRange.style.setProperty(
                    '--aifsm-period-start', (periodStart / maximum * 100) + '%'
                );
                dualRange.style.setProperty(
                    '--aifsm-period-end', (periodEnd / maximum * 100) + '%'
                );
            }
        }
        ...
        function mapLayerTitle(layer, step, sourceKey) {
            var title = (layer ? layer.label : 'Carte AIFS') +
        ...
        function parseVectorOverlay(source) {
                ...
                var paths = Array.from(svg.querySelectorAll('path')).map(
                    function (node) {
                        var role = node.getAttribute('data-aifsm-role') || '';
                        var arrowPoints = [];
                        var isobarLabels = [];
                        if (role === 'wind-arrows') {
                            arrowPoints = String(
                                node.getAttribute('data-aifsm-points') || ''
                        ...
                        if (role === 'isobar-labels') {
                            isobarLabels = String(
                                node.getAttribute('data-aifsm-labels') || ''
                        ...
                        return {
                            ...
                            hideAtDeepZoom:
                                node.getAttribute('data-aifsm-hide-deep') === '1',
                            role: role,
        ...
        app.addEventListener('aifsm:focus-location', function (event) {
        ...
        toolButtons.forEach(function (button) {
            button.addEventListener('click', function () {
                setToolMode(button.dataset.aifsmTool);
            });
        });
        if (captureButton) {
        ...
        viewport.addEventListener('pointerdown', function (event) {
            if (event.target.closest('button, .aifsm-diagram-popup, .aifsm-probe-pinned')) {
                return;
            }
            hideProbe();
            tapStart = {
        ...
        if (!baseUrl) {
            showError('Adresse des données AIFS non configurée.');
            return;
        }
        ...
        fetchJson(
            ...
            .catch(function (error) {
                showError('Les cartes AIFS ne sont pas encore publiées : ' + error.message);
            });
    }

    whenReady(function () {
        document.querySelectorAll('[data-aifsm-app]').forEach(initMap);
    });
}());
